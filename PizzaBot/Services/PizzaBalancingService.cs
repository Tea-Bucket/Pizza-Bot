...
namespace PizzaBot.Services
{
    public class PizzaBalancingService
    {
        ...
        public (Dictionary<int, PizzaResult> results, int requiredMeat, int requiredVeggie, int requiredVegan, float totalCost) Distribute(Dictionary<int, PizzaRequest> orders)
        {
            ...
            switch (_config!.PenaltyType) {
                case PenaltyType.Shape: {
                    var (results, config) = CompactBalance.Distribute<ShapeCost>(orders, (uint)_config.Fragments, _config.Price / 100f / _config.Fragments);
                ...
                case PenaltyType.PfeifferTreimerModified: {
                    var (results, config) = CompactBalance.Distribute<PfeifferTreimerModified>(orders, (uint)_config.Fragments, _config.Price / 100f / _config.Fragments);
                    return (results, (int)config.meat, (int)config.vegetarian, (int)config.vegan, config.Reduce(0u, (acc, val) => acc + val) * _config.Price / 100.0f);
                }
            }
